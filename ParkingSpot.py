...
class ParkingSpot:
    ...
    def draw_classes(self, image, overlay, alpha=0.2):
        if self.cluster != -1:
            ...
            cv2.circle(image, self.center, radius=3, color=(255, 255, 255), thickness=3)
    ...
    def visualize_cluster(self, image, overlay, index=None):
        ...
        if self.cluster != -1:
            ...
            if index is not None:
                ...
                text_color = (255, 255, 255) if getattr(self, 'cluster', -1) == -1 else (0, 0, 0)
                cv2.putText(image, str(index), text_pos, cv2.FONT_HERSHEY_SIMPLEX,
                            fontScale=2, color=text_color, thickness=12, lineType=cv2.LINE_AA)
